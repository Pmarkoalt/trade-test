...
from trading_system.ml.models import ModelType  # noqa: E402
...
def extract_features_and_labels(
    runner: BacktestRunner, period: str = "train"
...
def train_trade_quality_model(
    ...
    metrics = trainer.train(feature_rows=feature_rows, target_values=labels, model_id=model_name)
...
def train_win_probability_model(
    ...
    # Train model
    print(f"  Training on {len(feature_rows)} samples...")
    metrics = trainer.train(feature_rows=feature_rows, target_values=win_labels, model_id=model_name)
...
def compare_baseline_vs_ml(config_path: str, model_dir: Path, period: str = "validation"):
    """
...
def main():
    ...
    try:
        ...
        feature_rows, r_multiples, returns, wins = extract_features_and_labels(runner, period="train")
        ...
        trainer_win_prob, model_dir_win_prob = train_win_probability_model(
            feature_rows=feature_rows, wins=wins, model_type=ModelType.RANDOM_FOREST, model_name="win_probability_predictor"
        )
        ...
        _ = runner.run_backtest(period="validation")  # Evaluate on validation

        # Extract validation data
        val_feature_rows, val_r_multiples, val_returns, val_wins = extract_features_and_labels(runner, period="validation")

        if len(val_feature_rows) > 0:
            # Evaluate R-multiple model
            print("\nEvaluating R-multiple prediction model...")
            val_metrics_r_mult = trainer_r_multiple.evaluate(feature_rows=val_feature_rows, target_values=val_r_multiples)
            ...
            val_metrics_win = trainer_win_prob.evaluate(feature_rows=val_feature_rows, target_values=val_win_labels)
            print("  Validation metrics:")
        ...
        print("  2. Test models on validation/holdout periods")
        print("  3. Integrate into strategy configs (see above)")
        print("  4. Run backtests with ML-enabled configs")
        print("  5. Compare baseline vs ML-enhanced performance")
        ...
        print("   And that test fixtures are available")
