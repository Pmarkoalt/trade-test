...
class SimpleMovingAverageStrategy(StrategyInterface):
    ...
    def check_entry_triggers(self, features: FeatureRow) -> Tuple[Optional[BreakoutType], float]:
        ...
        """
        # Check if we have required indicators
        if features.ma_20 is None or features.ma_50 is None or features.close is None or features.prev_close is None:
            return None, 0.0

        # Check MA crossover: price crosses above 20-day MA
        price_crossed_above = features.prev_close <= features.ma_20 and features.close > features.ma_20
    ...
    def compute_stop_price(self, entry_price: float, entry_date, features: FeatureRow) -> Optional[float]:
    ...
    def compute_target_price(self, entry_price: float, entry_date, features: FeatureRow) -> Optional[float]:
...
def example_register_custom_strategy():
    ...
    register_strategy(strategy_type="sma_crossover", asset_class="equity", strategy_class=SimpleMovingAverageStrategy)
...
def example_create_custom_strategy_from_config():
    ...
    config_dict = {
        ...
        "benchmark": "SPY",
        "indicators": {"ma_periods": [20, 50], "atr_period": 14, "adv_lookback": 20},
        "entry": {"fast_clearance": 0.0},  # Not used for MA crossover
        "exit": {"mode": "ma_cross", "exit_ma": 20},
        "risk": {"risk_per_trade": 0.01, "max_positions": 5},  # 1% risk per trade
    }
