...
class TestFullBacktest:
    ...
    def test_full_backtest_run(self, test_config_path):
        ...
        assert 0.0 <= results['max_drawdown'] <= 1.0, (
            f"Max drawdown should be between 0 and 1, got {results['max_drawdown']}"
        )
        
        # Win rate should be between 0 and 1
        assert 0.0 <= results['win_rate'] <= 1.0, (
            f"Win rate should be between 0 and 1, got {results['win_rate']}"
        )
        ...
        assert results['starting_equity'] == config.portfolio.starting_equity, (
    ...
    def test_expected_trades(self, test_config_path):
            ...
            for trade in closed_trades:
                ...
                if trade.exit_date is not None:
                    assert trade.entry_date <= trade.exit_date, (
                        f"Entry date should be before exit date for {trade.symbol}"
                    )
        ...
        if portfolio.gross_exposure_pct is not None:
            assert portfolio.gross_exposure_pct <= 0.80, (
        ...
        if portfolio.per_position_exposure:
            for symbol, exposure_pct in portfolio.per_position_exposure.items():
                assert exposure_pct <= 0.15, (
                    f"Position {symbol} exposure should not exceed 15%, got {exposure_pct * 100:.2f}%"
                )
        ...
        if closed_trades:
            print(f"\nClosed Trades:")
            for trade in closed_trades:
                print(f"  {trade.symbol}: Entry {trade.entry_date.date()} @ ${trade.entry_price:.2f}, "
    ...
    def test_walk_forward_workflow(self, test_config_path, tmp_path):
        ...
        print(f"  Train: {train_results['total_trades']} trades, Sharpe: {train_results['sharpe_ratio']:.2f}, Return: {train_results['total_return']:.2%}")
        print(f"  Validation: {validation_results['total_trades']} trades, Sharpe: {validation_results['sharpe_ratio']:.2f}, Return: {validation_results['total_return']:.2%}")
        print(f"  Holdout: {holdout_results['total_trades']} trades, Sharpe: {holdout_results['sharpe_ratio']:.2f}, Return: {holdout_results['total_return']:.2%}")
        ...
        if total_trades >= 10:
            ...
            if validation_suite_results.get('status') == 'success':
                ...
                if 'bootstrap' in results:
                    ...
                    assert 'sharpe_5th' in bootstrap or 'sharpe_50th' in bootstrap, (
                ...
                if 'permutation' in results:
                    ...
                    assert 'actual_sharpe' in permutation or 'percentile_rank' in permutation, (
...
class TestEdgeCaseIntegration:
    ...
    def test_extreme_move_with_position(self):
        ...
        dates = pd.DatetimeIndex([
            "2023-01-01",
            "2023-01-02",  # Extreme move day (>50%)
            "2023-01-03"
        ])
        bars = pd.DataFrame({
            'open': [100.0, 101.0, 102.0],
