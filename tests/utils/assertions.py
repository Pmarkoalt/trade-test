...
def assert_no_lookahead(
    signals: List[Signal], bars: Dict[pd.Timestamp, Bar], feature_rows: Optional[Dict[pd.Timestamp, Any]] = None
) -> None:
...
def assert_valid_bar(bar: Bar) -> None:
    ...
    assert bar.low <= bar.high, f"Bar {bar.symbol} {bar.date}: low {bar.low} > high {bar.high}"
    assert bar.low <= bar.open <= bar.high, f"Bar {bar.symbol} {bar.date}: open {bar.open} not between low/high"
    assert bar.low <= bar.close <= bar.high, f"Bar {bar.symbol} {bar.date}: close {bar.close} not between low/high"
    ...
    if hasattr(bar, "dollar_volume") and bar.dollar_volume is not None:
        ...
        assert abs(bar.dollar_volume - expected_dv) < 0.01, (
            f"Bar {bar.symbol} {bar.date}: dollar_volume {bar.dollar_volume} " f"does not match close * volume {expected_dv}"
        )
...
def assert_valid_signal(signal: Signal) -> None:
    ...
    """
    # Check asset class
    assert signal.asset_class in ["equity", "crypto"], f"Signal {signal.symbol}: invalid asset_class {signal.asset_class}"

    # Check prices
    assert signal.entry_price > 0, f"Signal {signal.symbol}: entry_price must be positive"
    assert signal.stop_price > 0, f"Signal {signal.symbol}: stop_price must be positive"
    assert signal.stop_price < signal.entry_price, (
    ...
    # Check ADV
    assert signal.adv20 > 0, f"Signal {signal.symbol}: adv20 must be positive"
    ...
    assert isinstance(signal.eligibility_failures, list), f"Signal {signal.symbol}: eligibility_failures must be a list"
...
def assert_valid_order(order: Order) -> None:
    ...
    """
    # Check asset class
    assert order.asset_class in ["equity", "crypto"], f"Order {order.order_id}: invalid asset_class {order.asset_class}"

    # Check quantity
    assert order.quantity > 0, f"Order {order.order_id}: quantity must be positive"

    # Check prices
    assert order.expected_fill_price > 0, f"Order {order.order_id}: expected_fill_price must be positive"
    assert order.stop_price > 0, f"Order {order.order_id}: stop_price must be positive"
    ...
    assert order.execution_date >= order.date, (
        f"Order {order.order_id}: execution_date {order.execution_date} " f"must be >= order date {order.date}"
    )
...
def assert_valid_fill(fill: Fill) -> None:
    ...
    """
    # Check asset class
    assert fill.asset_class in ["equity", "crypto"], f"Fill {fill.fill_id}: invalid asset_class {fill.asset_class}"

    # Check quantity
    assert fill.quantity > 0, f"Fill {fill.fill_id}: quantity must be positive"

    # Check prices
    assert fill.fill_price > 0, f"Fill {fill.fill_id}: fill_price must be positive"
    assert fill.open_price > 0, f"Fill {fill.fill_id}: open_price must be positive"

    # Check slippage direction matches side
    if fill.side.value == "BUY":
        assert fill.fill_price >= fill.open_price, (
            f"Fill {fill.fill_id}: BUY fill_price {fill.fill_price} should be " f">= open_price {fill.open_price}"
        )
    else:  # SELL
        assert fill.fill_price <= fill.open_price, (
            f"Fill {fill.fill_id}: SELL fill_price {fill.fill_price} should be " f"<= open_price {fill.open_price}"
        )

    # Check costs
    assert fill.slippage_bps >= 0, f"Fill {fill.fill_id}: slippage_bps must be non-negative"
    assert fill.fee_bps >= 0, f"Fill {fill.fill_id}: fee_bps must be non-negative"

    # Check notional
    expected_notional = fill.fill_price * fill.quantity
    assert abs(fill.notional - expected_notional) < 0.01, (
        f"Fill {fill.fill_id}: notional {fill.notional} does not match " f"fill_price * quantity {expected_notional}"
    )
...
def assert_valid_portfolio(portfolio: Portfolio) -> None:
    ...
    """
    # Check equity
    assert portfolio.starting_equity > 0, f"Portfolio {portfolio.date}: starting_equity must be positive"
    assert portfolio.equity > 0, f"Portfolio {portfolio.date}: equity must be positive"
    assert portfolio.cash >= 0, f"Portfolio {portfolio.date}: cash must be non-negative"
    ...
    assert portfolio.open_trades >= 0, f"Portfolio {portfolio.date}: open_trades must be non-negative"
    assert portfolio.total_trades >= 0, f"Portfolio {portfolio.date}: total_trades must be non-negative"
    ...
    # Check equity curve
    assert len(portfolio.equity_curve) > 0, f"Portfolio {portfolio.date}: equity_curve must not be empty"
...
def assert_trades_match_expected(
    actual_trades: List[Dict[str, Any]], expected_trades: List[Dict[str, Any]], tolerance: float = 0.01
) -> None:
    ...
    assert len(actual_trades) == len(expected_trades), (
        f"Number of trades mismatch: actual {len(actual_trades)}, " f"expected {len(expected_trades)}"
    )

    for i, (actual, expected) in enumerate(zip(actual_trades, expected_trades)):
        # Check symbol
        assert actual["symbol"] == expected["symbol"], (
            f"Trade {i}: symbol mismatch: actual {actual['symbol']}, " f"expected {expected['symbol']}"
        )

        # Check dates
        if "entry_date" in expected:
            assert actual["entry_date"] == expected["entry_date"], (
                f"Trade {i}: entry_date mismatch: actual {actual['entry_date']}, " f"expected {expected['entry_date']}"
            )

        # Check prices (with tolerance)
        if "entry_price" in expected:
            assert abs(actual["entry_price"] - expected["entry_price"]) < tolerance, (
                f"Trade {i}: entry_price mismatch: actual {actual['entry_price']}, " f"expected {expected['entry_price']}"
            )

        if "exit_price" in expected:
            assert abs(actual["exit_price"] - expected["exit_price"]) < tolerance, (
                f"Trade {i}: exit_price mismatch: actual {actual['exit_price']}, " f"expected {expected['exit_price']}"
            )
