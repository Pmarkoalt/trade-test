...
class SyntheticDataGenerator:
    ...
    def generate_ohlcv(
        ...
        **kwargs
        ...
        if pattern == DataPattern.EXTREME_MOVE:
            return self._generate_extreme_move(
                symbol, dates, base_price, base_volume, **kwargs
            )
        elif pattern == DataPattern.FLASH_CRASH:
            return self._generate_flash_crash(
                symbol, dates, base_price, base_volume, **kwargs
            )
        elif pattern == DataPattern.BREAKOUT_FAST:
            return self._generate_breakout(
                symbol, dates, base_price, base_volume, BreakoutType.FAST_20D, **kwargs
            )
        elif pattern == DataPattern.BREAKOUT_SLOW:
            return self._generate_breakout(
                symbol, dates, base_price, base_volume, BreakoutType.SLOW_55D, **kwargs
            )
        elif pattern == DataPattern.HIGH_VOLATILITY:
            return self._generate_ohlcv_base(
                symbol, dates, base_price, base_volume, volatility=0.05, trend=trend
            )
        elif pattern == DataPattern.LOW_VOLATILITY:
            return self._generate_ohlcv_base(
                symbol, dates, base_price, base_volume, volatility=0.005, trend=trend
            )
        elif pattern == DataPattern.MISSING_DAYS:
            return self._generate_with_missing_days(
                symbol, dates, base_price, base_volume, volatility, trend, **kwargs
            )
        elif pattern == DataPattern.INVALID_OHLC:
            return self._generate_invalid_ohlc(
                symbol, dates, base_price, base_volume, **kwargs
            )
        else:
            # Normal pattern
            return self._generate_ohlcv_base(
                symbol, dates, base_price, base_volume, volatility, trend
            )
    ...
    def _generate_with_missing_days(
        ...
        full_df = self._generate_ohlcv_base(
            symbol, dates, base_price, base_volume, volatility, trend
        )
    ...
    def _generate_invalid_ohlc(
        ...
        normal_df = self._generate_ohlcv_base(
            symbol, dates, base_price, base_volume, 0.02, 0.0
        )
    ...
    def generate_large_dataset(
        ...
        for symbol in symbols:
            df = self._generate_ohlcv_base(
                symbol, dates, base_prices[symbol], 50000000.0, 0.02, 0.0
            )
            result[symbol] = df
...
def generate_trend_data(
    ...
    return generator.generate_ohlcv(
        symbol, start_date, end_date, base_price, trend=trend
    )
...
def generate_breakout_data(
    ...
    return generator.generate_ohlcv(
        symbol, start_date, end_date, base_price, pattern=pattern
    )
...
def generate_edge_case_data(
    ...
    **kwargs
    ...
    return generator.generate_ohlcv(
        symbol, start_date, end_date, pattern=pattern, **kwargs
    )
