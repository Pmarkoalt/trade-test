...
def create_sample_bar(
    date: pd.Timestamp, symbol: str = "AAPL", base_price: float = 150.0, volatility: float = 0.02, volume: float = 50000000.0
) -> Bar:
...
def create_sample_position(
    symbol: str, asset_class: str, entry_date: pd.Timestamp, entry_price: float, quantity: int, stop_price: float, **kwargs
) -> Position:
    ...
    Args:
        ...
        stop_price: Stop price
        **kwargs: Additional position parameters (including 'side' for LONG/SHORT)
    ...
    if isinstance(side_str, str):
        side = PositionSide.SHORT if side_str.upper() == "SHORT" else PositionSide.LONG
    else:
        side = side_str if isinstance(side_str, PositionSide) else PositionSide.LONG
...
def create_sample_portfolio(
    date: pd.Timestamp, starting_equity: float = 100000.0, cash: Optional[float] = None, **kwargs
) -> Portfolio:
...
def create_mock_market_data(
    symbols: List[str], start_date: pd.Timestamp, end_date: pd.Timestamp, base_prices: Optional[Dict[str, float]] = None
) -> MarketData:
    ...
    for symbol in symbols:
        ...
        for date in dates:
            bar = create_sample_bar(date=date, symbol=symbol, base_price=current_price, volatility=0.02, volume=50000000.0)
            bars_dict[date] = bar
