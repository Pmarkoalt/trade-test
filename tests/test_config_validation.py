...
class TestExampleConfigsValidation:
    ...
    def test_example_config_validates(self, config_path):
        ...
        # Validate the config
        is_valid, error_message, config = validate_config_file(
            str(full_path),
    ...
    def test_all_example_configs_validate(self, example_configs):
        ...
        if failed_configs:
            ...
            pytest.fail(
                f"The following config files failed validation:\n\n{error_summary}"
            )
    ...
    def test_run_config_references_valid_strategy_configs(self):
        ...
        # Validate run config
        is_valid, error_message, run_config = validate_config_file(
            str(run_config_path),
            config_type="run"
        )
        ...
        if isinstance(run_config, RunConfig):
            ...
            if run_config.strategies.equity and run_config.strategies.equity.enabled:
                ...
                if equity_path.exists():
                    ...
                    assert equity_valid, (
                        f"Equity strategy config referenced by run_config.yaml "
                        f"failed validation: {equity_error}"
                    )
            ...
            if run_config.strategies.crypto and run_config.strategies.crypto.enabled:
                ...
                if crypto_path.exists():
                    ...
                    assert crypto_valid, (
                        f"Crypto strategy config referenced by run_config.yaml "
                        f"failed validation: {crypto_error}"
                    )
