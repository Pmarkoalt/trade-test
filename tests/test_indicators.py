...
from trading_system.indicators import (
    ma, atr, roc, highest_close, adv, rolling_corr,
...
class TestATR:
    ...
    def test_atr_basic(self):
        ...
        df = pd.DataFrame({
            ...
            'low': [99, 100, 99.5, 101, 102],
            'close': [100.5, 101, 100, 102, 103]
        }, index=dates[:5])
...
class TestROC:
    ...
    def test_roc_basic(self):
        ...
        close = pd.Series([100 * (1.01 ** i) for i in range(100)],
...
class TestADV:
    ...
    def test_adv_basic(self):
        ...
        dollar_vol = pd.Series([1e6 * (1 + i * 0.01) for i in range(30)],
                              index=pd.date_range('2024-01-01', periods=30))
...
class TestRollingCorrelation:
    ...
    def test_rolling_corr_perfect_correlation(self):
        ...
        returns_a = pd.Series([0.01, 0.02, -0.01, 0.03] * 10,
    ...
    def test_rolling_corr_misaligned_indices(self):
        ...
        returns_a = pd.Series([0.01, 0.02, 0.03],
                              index=pd.date_range('2024-01-01', periods=3))
        returns_b = pd.Series([0.02, 0.03, 0.04],
                             index=pd.date_range('2024-01-02', periods=3))  # Different dates
...
class TestComputeFeatures:
    ...
    def test_compute_features_with_benchmark(self):
        ...
        features = compute_features(
            df, symbol='BTC', asset_class='crypto',
            benchmark_roc60=benchmark_roc,
