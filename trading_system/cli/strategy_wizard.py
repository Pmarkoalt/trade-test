...
class StrategyWizard:
    ...
    def run_wizard(self) -> Dict[str, Any]:
        ...
        strategy_name = self.prompt(
            "Strategy name (lowercase with underscores, e.g., 'my_custom_strategy')",
            required=True
        )
        ...
        strategy_type = self.choose_option(
        ...
        asset_class = self.choose_option(
            "Select asset class",
            asset_classes,
            default="equity"
        )
        ...
        use_custom_path = self.confirm(
            "Use custom output path? (otherwise auto-generate in strategies directory)",
            default=False
        )
        ...
        if use_custom_path:
            output_path = self.prompt(
                "Output file path",
                required=True
            )
        else:
            use_custom_dir = self.confirm(
                "Use custom directory? (otherwise use default based on strategy type)",
                default=False
            )
            if use_custom_dir:
