...
class ConfigWizard:
    ...
    def _create_run_config(self) -> Dict[str, Any]:
        ...
        config["portfolio"] = {
            "starting_equity": float(self.prompt("Starting equity (USD)", default="100000.0"))
        }
    ...
    def _create_strategy_config(self) -> Dict[str, Any]:
        ...
        config["benchmark"] = self.prompt(
            "Benchmark symbol",
            default="SPY" if config["asset_class"] == "equity" else "BTC"
        )
        ...
        config["capacity"] = {
            "max_order_pct_adv": 0.005 if config["asset_class"] == "equity" else 0.0025
        }
