...
def roc(close: pd.Series, window: int = 60, use_cache: bool = True) -> pd.Series:
    ...
    if use_cache:
        ...
        if cache is not None:
            cache_key = (f"roc_{id(close)}_{len(close)}_{close.iloc[-1] if len(close) > 0 else 0}", "roc", window)
