...
from .cache import create_cache_key_for_dataframe, get_cache


def atr(df_ohlc: pd.DataFrame, period: int = 14, use_cache: bool = True, symbol: Optional[str] = None) -> pd.Series:
    ...
    if not all(col in df_ohlc.columns for col in required_cols):
        raise ValueError(f"DataFrame must contain columns: {required_cols}. " f"Found: {list(df_ohlc.columns)}")

    # Check cache if enabled
    if use_cache:
        cache = get_cache()
        if cache is not None:
            cache_key = create_cache_key_for_dataframe(df_ohlc, "atr", period, symbol=symbol)
