...
def compute_features(
    ...
    if not all(col in df_ohlc.columns for col in required_cols):
        raise IndicatorError(
            f"DataFrame must contain columns: {required_cols}. " f"Found: {list(df_ohlc.columns)}",
    ...
    if asset_class not in ["equity", "crypto"]:
        raise IndicatorError(
            f"asset_class must be 'equity' or 'crypto', got: {asset_class}", indicator_name="compute_features", symbol=symbol
        )
    ...
    dollar_volume = (
        df_ohlc["close"] * df_ohlc["volume"] if "dollar_volume" not in df_ohlc.columns else df_ohlc["dollar_volume"]
    )

    # Compute all indicators in batch (vectorized operations)
    # Moving averages - compute all at once using same close series
    # Pass symbol for better cache keys (enables cross-strategy caching)
    close_series = df_ohlc["close"]
    ma20 = ma(close_series, window=20, use_cache=use_cache, symbol=symbol)
    ma50 = ma(close_series, window=50, use_cache=use_cache, symbol=symbol)
    ma200 = ma(close_series, window=200, use_cache=use_cache, symbol=symbol)

    # ATR - requires high, low, close
    atr14 = atr(df_ohlc[["high", "low", "close"]], period=14, use_cache=use_cache, symbol=symbol)

    # ROC60
    roc60 = roc(close_series, window=60, use_cache=use_cache, symbol=symbol)

    # Highest close (excluding today) - compute both windows
    highest_close_20d = highest_close(close_series, window=20, use_cache=use_cache, symbol=symbol)
    highest_close_55d = highest_close(close_series, window=55, use_cache=use_cache, symbol=symbol)

    # ADV20
    adv20 = adv(dollar_volume, window=20, use_cache=use_cache, symbol=symbol)
    ...
    returns_1d = pd.Series(
        np.divide(
            close_series.values, close_shifted.values, out=np.full(len(close_series), np.nan), where=close_shifted.values != 0
        )
        - 1,
    ...
    result = pd.DataFrame(
        {
            ...
            "close": (
                df_ohlc["close"].values.astype("float32") if df_ohlc["close"].dtype != "float32" else df_ohlc["close"].values
            ),
...
def compute_features_for_date(features_df: pd.DataFrame, date: pd.Timestamp) -> Optional[FeatureRow]:
    ...
    try:
        ...
        raise IndicatorError(
            f"Date {date} not found in features DataFrame", indicator_name="compute_features_for_date", symbol=None
        ) from e
