...
class Portfolio:
    ...
    def update_correlation_metrics(
        self, returns_data: Dict[str, List[float]], lookback: int = 20
    ) -> None:
    ...
    def __post_init__(self):
        ...
        if self.starting_equity <= 0:
            raise ValueError(
                f"Invalid starting_equity: {self.starting_equity}, must be positive"
            )
        ...
        if not (0.33 <= self.risk_multiplier <= 1.0):
            raise ValueError(
                f"Invalid risk_multiplier: {self.risk_multiplier}, must be between 0.33 and 1.0"
            )
