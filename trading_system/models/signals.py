...
class Signal:
    ...
    def __post_init__(self):
        ...
        # Validate stop price (for long positions, stop must be below entry)
        if self.signal_type in [SignalType.ENTRY_LONG, SignalType.EXIT]:
    ...
    def is_valid(self) -> bool:
        ...
        if self.signal_type in [SignalType.ENTRY_LONG, SignalType.ENTRY_SHORT]:
            return (
                ...
                self.side == SignalSide.BUY if self.signal_type == SignalType.ENTRY_LONG else self.side == SignalSide.SELL
            )
