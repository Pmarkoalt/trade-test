...
class Order:
    ...
    def __post_init__(self):
        ...
        if self.expected_fill_price <= 0:
            raise ValueError(
                f"Invalid expected_fill_price: {self.expected_fill_price}, must be positive"
            )
    ...
    def __post_init__(self):
        ...
        if self.side == SignalSide.BUY:
            ...
            if self.fill_price < self.open_price:
                raise ValueError(
                    f"Invalid fill_price for BUY: {self.fill_price} < open_price {self.open_price}"
                )
        else:  # SELL
            # SELL: fill_price should be <= open_price (receiving less)
            if self.fill_price > self.open_price:
                raise ValueError(
                    f"Invalid fill_price for SELL: {self.fill_price} > open_price {self.open_price}"
                )
