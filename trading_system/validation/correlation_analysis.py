...
class CorrelationStressAnalysis:
    ...
    def __init__(self, portfolio_history: Dict[pd.Timestamp, Dict], returns_data: Dict[str, pd.Series], lookback: int = 20):
    ...
    def run(self) -> Dict:
        ...
        drawdowns = [(eq - peak) / peak if peak > 0 else 0.0 for eq, peak in zip(equities, peaks)]
        ...
        for date, dd in zip(dates, drawdowns):
            ...
            corr_matrix = self._compute_correlation_matrix(position_symbols, date, self.lookback)
    ...
    def _compute_correlation_matrix(self, symbols: List[str], date: pd.Timestamp, lookback: int) -> Optional[np.ndarray]:
...
def run_correlation_stress_analysis(
    portfolio_history: Dict[pd.Timestamp, Dict], returns_data: Dict[str, pd.Series], lookback: int = 20
) -> Dict:
    ...
    """
    analysis = CorrelationStressAnalysis(portfolio_history, returns_data, lookback)
    return analysis.run()
