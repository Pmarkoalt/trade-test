...
def compute_sharpe_from_r_multiples(
...
class BootstrapTest:
    ...
    def run(self) -> Dict:
        ...
        original_sharpe = compute_sharpe_from_r_multiples(
            self.r_multiples.tolist(),
        ...
        original_calmar = (
            original_sharpe / abs(original_max_dd) if original_max_dd != 0 else 0.0
        )
...
def check_bootstrap_results(results: Dict) -> Tuple[bool, List[str]]:
    ...
    if results['sharpe_percentile_rank'] < 40 or results['sharpe_percentile_rank'] > 60:
        warnings.append(
            f"WARNING: Original Sharpe at {results['sharpe_percentile_rank']:.1f}th "
            f"percentile (unusual)"
        )
