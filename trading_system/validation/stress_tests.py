...
class StressTestSuite:
    ...
    def __init__(self, run_backtest_func: Callable, random_seed: Optional[int] = None):
    ...
    def run_flash_crash_simulation(self) -> Dict:
        ...
        return {"simulation": "flash_crash", "results": results, "max_dd": max_dd, "survived": max_dd > -0.25}  # DD < 25%


def run_slippage_stress(run_backtest_func: Callable, multiplier: float, random_seed: Optional[int] = None) -> Dict:
    """Run slippage stress test.
...
def run_bear_market_test(run_backtest_func: Callable, random_seed: Optional[int] = None) -> Dict:
...
def run_range_market_test(run_backtest_func: Callable, random_seed: Optional[int] = None) -> Dict:
...
def run_flash_crash_simulation(run_backtest_func: Callable, random_seed: Optional[int] = None) -> Dict:
    """Run flash crash simulation.
...
def check_stress_results(stress_results: Dict[str, Dict]) -> Tuple[bool, List[str]]:
    ...
    if "slippage_2x" in stress_results:
        ...
        if sharpe <= 0.75:
            all_passed = False
            warnings.append(f"REJECT: 2x slippage Sharpe {sharpe:.2f} <= 0.75")
    ...
    if "slippage_3x" in stress_results:
        ...
        if calmar <= 1.0:
            all_passed = False
            warnings.append(f"REJECT: 3x slippage Calmar {calmar:.2f} <= 1.0")
    ...
    if "bear_market" in stress_results:
        ...
        if max_dd < -0.25:  # DD > 25%
            all_passed = False
            warnings.append(f"REJECT: Bear market Max DD {max_dd:.1%} > 25%")
    ...
    if "range_market" in stress_results:
        ...
        if max_dd < -0.15:  # DD > 15%
            warnings.append(f"WARNING: Range market Max DD {max_dd:.1%} > 15%")
        if total_return < -0.05:  # Return < -5%
            warnings.append(f"WARNING: Range market return {total_return:.1%} < -5%")
    ...
    if "flash_crash" in stress_results:
        ...
        if not survived or max_dd < -0.25:  # DD > 25%
            all_passed = False
            warnings.append(f"REJECT: Flash crash Max DD {max_dd:.1%} > 25% or portfolio did not survive")
