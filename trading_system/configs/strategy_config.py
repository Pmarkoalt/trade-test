...
from .validation import ConfigValidationError, validate_file_exists, validate_yaml_format, wrap_validation_error
...
class ExitConfig(BaseModel):
    ...
    def validate_staged_exit(self):
        ...
        if self.tightened_stop_atr_mult is not None and self.tightened_stop_atr_mult >= self.hard_stop_atr_mult:
            raise ValueError(
                f"tightened_stop_atr_mult ({self.tightened_stop_atr_mult}) must be < hard_stop_atr_mult ({self.hard_stop_atr_mult})"
            )
        return self
...
class RiskConfig(BaseModel):
    ...
    max_exposure: float = Field(default=0.80, gt=0.0, le=1.0, description="80% max gross exposure")
    max_position_notional: float = Field(default=0.15, gt=0.0, le=1.0, description="15% max position size")

    # Short selling limits
    max_net_exposure: Optional[float] = Field(
        default=None, gt=0.0, le=1.0, description="Max net exposure (long - short) as fraction of equity, e.g., 0.40 for ±40%"
    )
    max_long_exposure: Optional[float] = Field(
        default=None, gt=0.0, le=1.0, description="Max long exposure as fraction of equity"
    )
    max_short_exposure: Optional[float] = Field(
        default=None, gt=0.0, le=1.0, description="Max short exposure as fraction of equity"
    )
    ...
    def validate_exposure(self):
        if self.max_position_notional > self.max_exposure:
            raise ValueError(
                f"max_position_notional ({self.max_position_notional}) should not exceed max_exposure ({self.max_exposure})"
            )

        # Validate short limits
        if self.max_long_exposure is not None and self.max_long_exposure > self.max_exposure:
            raise ValueError(
                f"max_long_exposure ({self.max_long_exposure}) should not exceed max_exposure ({self.max_exposure})"
            )

        if self.max_short_exposure is not None and self.max_short_exposure > self.max_exposure:
            raise ValueError(
                f"max_short_exposure ({self.max_short_exposure}) should not exceed max_exposure ({self.max_exposure})"
            )

        if self.max_net_exposure is not None:
            if self.max_long_exposure is not None and self.max_net_exposure > self.max_long_exposure:
                raise ValueError(
                    f"max_net_exposure ({self.max_net_exposure}) should not exceed max_long_exposure ({self.max_long_exposure})"
                )
            if self.max_short_exposure is not None and self.max_net_exposure > self.max_short_exposure:
                raise ValueError(
                    f"max_net_exposure ({self.max_net_exposure}) should not exceed max_short_exposure ({self.max_short_exposure})"
                )
...
class StrategyConfig(BaseModel):
    ...
    def from_yaml(cls, path: str) -> "StrategyConfig":
        ...
        try:
            ...
            raise ConfigValidationError(f"Failed to load strategy configuration: {str(e)}", config_path=path) from e
