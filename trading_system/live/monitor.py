...
from ..models.signals import Signal, BreakoutType
...
class LiveMonitor:
    ...
    def _execute_exit(self, symbol: str, exit_reason: ExitReason, exit_price: Optional[float]) -> None:
        ...
        try:
            ...
            if closed_position:
                ...
                self._add_alert(
                    AlertLevel.INFO,
                    f"Position exited: {symbol} ({exit_reason.value})",
                    symbol=symbol
                )
        except Exception as e:
            ...
            self._add_alert(
                AlertLevel.WARNING,
                f"Failed to exit position {symbol}: {e}",
                symbol=symbol
            )
    ...
    def execute_signal(self, signal: Signal) -> Optional[Fill]:
        ...
        if qty < 1:
            ...
            self._add_alert(
                AlertLevel.WARNING,
                f"Cannot afford position for {signal.symbol}",
        ...
        try:
            ...
            logger.info(
                f"Executed signal {signal.symbol}: {qty} shares @ ${fill.fill_price:.2f} | "
            ...
            self._add_alert(
                AlertLevel.WARNING,
                f"Failed to execute order for {signal.symbol}: {e}",
                symbol=signal.symbol
            )
            return None
