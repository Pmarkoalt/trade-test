...
def log_trade_event(
    ...
    **kwargs
    ...
    if event_type == TradeEventType.ENTRY:
        logger.info(f"TRADE_ENTRY: {symbol} | Price: {kwargs.get('entry_price', 'N/A')} | "
                   f"Qty: {kwargs.get('quantity', 'N/A')} | Stop: {kwargs.get('stop_price', 'N/A')} | "
                   f"Trigger: {kwargs.get('triggered_on', 'N/A')}", extra=event_data)
    elif event_type == TradeEventType.EXIT:
        logger.info(f"TRADE_EXIT: {symbol} | Exit Price: {kwargs.get('exit_price', 'N/A')} | "
                   f"Reason: {kwargs.get('exit_reason', 'N/A')} | "
                   f"P&L: {kwargs.get('realized_pnl', 'N/A'):.2f} | "
                   f"R-Multiple: {kwargs.get('r_multiple', 'N/A'):.2f}", extra=event_data)
    elif event_type == TradeEventType.STOP_HIT:
        logger.warning(f"TRADE_STOP_HIT: {symbol} | Stop Price: {kwargs.get('stop_price', 'N/A')} | "
                      f"Exit Price: {kwargs.get('exit_price', 'N/A')} | "
                      f"P&L: {kwargs.get('realized_pnl', 'N/A'):.2f}", extra=event_data)
...
def log_signal_generation(
    ...
    **kwargs
    ...
    if signal_generated:
        logger.debug(f"SIGNAL_GENERATED: {symbol} | Trigger: {kwargs.get('triggered_on', 'N/A')} | "
                    f"Clearance: {kwargs.get('clearance', 'N/A'):.4f} | "
                    f"Score: {kwargs.get('score', 'N/A'):.2f}", extra=signal_data)
    else:
...
def log_portfolio_snapshot(
    ...
    **kwargs
    ...
    logger.info(f"PORTFOLIO_SNAPSHOT: {date} | Equity: ${equity:,.2f} | "
               f"Cash: ${cash:,.2f} | Positions: {open_positions} | "
               f"Realized P&L: ${realized_pnl:,.2f} | Unrealized P&L: ${unrealized_pnl:,.2f} | "
               f"Exposure: ${gross_exposure:,.2f} | Risk Mult: {risk_multiplier:.2f}", extra=snapshot_data)
