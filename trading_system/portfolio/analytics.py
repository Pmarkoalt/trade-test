...
class PortfolioAnalyticsCalculator:
    ...
    def calculate_risk_attribution(
        ...
        for i, symbol in enumerate(available_symbols):
            attribution.append(RiskAttribution(
                asset=symbol,
    ...
    def calculate_performance_attribution(
        ...
        if portfolio_return is None:
            portfolio_return = sum(
                weights.get(s, 0.0) * asset_returns.get(s, 0.0)
                for s in set(weights.keys()) | set(asset_returns.keys())
            )
        ...
        for symbol in set(weights.keys()) | set(asset_returns.keys()):
            ...
            attribution.append(PerformanceAttribution(
                asset=symbol,
                weight=weight,
    ...
    def calculate_full_analytics(
        ...
        """
        # Basic metrics
        total_return, ann_return, vol, sharpe, max_dd, calmar = self.calculate_basic_metrics(
            equity_curve, returns
        )
        
        # Risk metrics
        var, cvar, beta, alpha, tracking_error = self.calculate_risk_metrics(
