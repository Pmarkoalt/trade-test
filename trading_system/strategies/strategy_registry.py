...
from ..exceptions import StrategyNotFoundError, StrategyError
...
try:
    ...
    PairsTradingStrategy = None  # Pairs strategy may not exist yet
...
def register_strategy(
    ...
    if not issubclass(strategy_class, StrategyInterface):
        raise ValueError(
            f"Strategy class must implement StrategyInterface, got {strategy_class}"
        )
    
    key = (strategy_type, asset_class)
    if key in _STRATEGY_REGISTRY:
        raise ValueError(
            f"Strategy already registered for type='{strategy_type}', asset_class='{asset_class}'"
        )
...
def get_strategy_class(
    strategy_type: str,
    asset_class: str
) -> Optional[Type[StrategyInterface]]:
