...
class PairsTradingStrategy(StrategyInterface):
    ...
    def check_eligibility(self, features: FeatureRow) -> tuple[bool, List[str]]:
        ...
        is_in_pair = any(
            features.symbol in pair for pair in self.pairs
        )
    ...
    def check_exit_signals(
        ...
        Note: Hard stop logic assumes long positions for now.
        Short positions would need portfolio manager support.
        ...
        """
        # Priority 1: Hard stop (assuming long position for now)
        # For short positions, this would be: features.close >= position.stop_price
    ...
    def check_pair_exit(
        ...
        if abs(spread_zscore) < self.exit_zscore:
            ...
            if position1 and position1.is_open():
                ...
                # For now, assume long positions (short support needs portfolio manager update)
