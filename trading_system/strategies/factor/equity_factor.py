...
class EquityFactorStrategy(FactorBaseStrategy):
    ...
    def __init__(self, config: StrategyConfig):
        ...
        if config.asset_class != "equity":
            raise ValueError(
                f"EquityFactorStrategy requires asset_class='equity', got '{config.asset_class}'"
            )
    ...
    def compute_factor_score(self, features: FeatureRow) -> Optional[float]:
        ...
        composite_score = (
            self.factors['momentum'] * momentum_raw +
            self.factors['value'] * value_raw +
            self.factors['quality'] * quality_raw
        )
    ...
    def _update_top_decile(self, current_date: datetime) -> None:
        ...
        sorted_symbols = sorted(
            self._factor_scores_cache.items(),
            key=lambda x: x[1],
            reverse=True
        )
