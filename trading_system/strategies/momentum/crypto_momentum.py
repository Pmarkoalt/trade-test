...
class CryptoMomentumStrategy(MomentumBaseStrategy):
    """Crypto momentum strategy with staged exit logic.
    ...
    def __init__(self, config: StrategyConfig):
        ...
        if config.asset_class != "crypto":
            raise ValueError(f"CryptoMomentumStrategy requires asset_class='crypto', got '{config.asset_class}'")

        super().__init__(config)

        # Validate exit mode
        if config.exit.mode != "staged":
            raise ValueError(f"CryptoMomentumStrategy requires exit.mode='staged', got '{config.exit.mode}'")
        ...
        if not position.tightened_stop and features.ma20 is not None and not np.isnan(features.ma20):
            if features.close < features.ma20:
                ...
                if features.atr14 is not None and not np.isnan(features.atr14):
                    tightened_stop = position.entry_price - (self.config.exit.tightened_stop_atr_mult * features.atr14)
    ...
    def should_tighten_stop(self, position: Position, features: FeatureRow) -> bool:
