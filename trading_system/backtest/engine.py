...
def create_backtest_engine_from_config(
    ...
    equity_config_path = (
        config.strategies.equity.config_path
        if config.strategies.equity and config.strategies.equity.enabled
        else None
    )
    crypto_config_path = (
        config.strategies.crypto.config_path
    ...
    logger.info(
        f"Market data loaded: {len(market_data.bars)} symbols, "
        f"{len(market_data.benchmarks)} benchmarks"
    )
