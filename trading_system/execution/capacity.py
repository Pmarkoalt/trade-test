...
def check_capacity_constraint(
    order_notional: float, adv20: float, asset_class: str, max_order_pct_adv: Optional[float] = None
) -> Tuple[bool, str]:
    """
...
def check_capacity_constraint_with_quantity(
    quantity: int, price: float, adv20: float, asset_class: str, max_order_pct_adv: Optional[float] = None
) -> Tuple[bool, str]:
    """
    ...
    return check_capacity_constraint(
        order_notional=order_notional, adv20=adv20, asset_class=asset_class, max_order_pct_adv=max_order_pct_adv
    )
