...
def validate_ohlcv(df: pd.DataFrame, symbol: str) -> bool:
    ...
    if invalid_ohlc.any():
        ...
        logger.error(
            f"{symbol}: Invalid OHLC at dates: {invalid_dates[:10]} " f"(showing first 10 of {invalid_ohlc.sum()} total)"
        )
        return False
    ...
    if df.index.duplicated().any():
        ...
        logger.error(f"{symbol}: Duplicate dates: {duplicates[:10]} " f"(showing first 10 of {len(duplicates)} total)")
        return False
...
def detect_missing_data(
    df: pd.DataFrame, symbol: str, asset_class: str = "equity", expected_frequency: str = "D"
) -> Dict[str, Any]:
    """
    ...
    """
    if len(df) == 0:
        return {"missing_dates": [], "consecutive_gaps": [], "gap_lengths": []}
    ...
    return {"missing_dates": missing_dates.tolist(), "consecutive_gaps": consecutive_gaps, "gap_lengths": gap_lengths}
