...
class UniverseConfig:
    ...
    def __post_init__(self):
        ...
        if self.mode == "dynamic" and not any([
            self.min_market_cap_usd,
            self.min_volume_usd,
            self.min_liquidity_score
        ]):
            raise ValueError(
                "dynamic mode requires at least one filter: "
                "min_market_cap_usd, min_volume_usd, or min_liquidity_score"
            )
...
class CryptoUniverseManager:
    ...
    def _select_fixed_universe(
        ...
        logger.info(
            f"Fixed universe: {len(selected)}/{len(fixed_list)} symbols "
            f"have data: {selected}"
        )
        
        return selected
    
    def _select_custom_universe(
        self, available_data: Dict[str, pd.DataFrame]
    ) -> List[str]:
        ...
        if self.config.universe_file_path:
            ...
            if not file_path.exists():
                raise FileNotFoundError(
                    f"Universe file not found: {self.config.universe_file_path}"
                )
            ...
            try:
                ...
                raise ValueError(
                    f"Error loading universe from {self.config.universe_file_path}: {e}"
                )
        ...
        logger.info(
            f"Custom universe: {len(selected)}/{len(symbols)} symbols "
            f"have data: {selected}"
        )
    ...
    def _select_dynamic_universe(
        ...
        if self.config.min_market_cap_usd is not None:
            ...
            if metrics_df['market_cap'].notna().any():
                filtered = filtered[
                    ...
                    (filtered['market_cap'] >= self.config.min_market_cap_usd)
                ]
            else:
                logger.warning(
                    "min_market_cap_usd filter specified but no market cap data available"
                )
        
        if self.config.min_volume_usd is not None:
            filtered = filtered[
                filtered['avg_dollar_volume'] >= self.config.min_volume_usd
            ]
        
        if self.config.min_liquidity_score is not None:
            filtered = filtered[
                filtered['liquidity_score_normalized'] >= self.config.min_liquidity_score
            ]
        
        # Sort by market cap (descending) or dollar volume if market cap not available
        if filtered['market_cap'].notna().any():
            filtered = filtered.sort_values(
                'market_cap', ascending=False, na_last=True
            )
        else:
        ...
        logger.info(
            f"Dynamic universe: selected {len(selected)}/{len(available_data)} symbols. "
            f"Selected: {selected}"
        )
        
        if len(selected) == 0:
            logger.warning(
                "Dynamic universe selection resulted in empty universe. "
    ...
    def should_rebalance(self, current_date: pd.Timestamp) -> bool:
        ...
        if self.config.rebalance_frequency == "monthly":
            ...
            return (
                current_date.year != self.last_rebalance_date.year or
    ...
    def rebalance_universe(
        ...
        if self.should_rebalance(current_date):
            ...
            logger.info(
                f"Universe rebalanced on {current_date.date()}: "
                f"{len(new_universe)} symbols"
            )
            ...
            if not self.current_universe:
                ...
                self.last_rebalance_date = current_date
                logger.info(
                    f"Universe initialized on {current_date.date()}: "
                    f"{len(self.current_universe)} symbols"
                )
    ...
    def validate_universe(
        ...
        if missing_data:
            warnings.append(
                f"Symbols in universe but missing data: {missing_data}"
            )
        ...
        if low_quality:
            warnings.append(
                f"Symbols with insufficient data (<20 bars): {low_quality}"
            )
