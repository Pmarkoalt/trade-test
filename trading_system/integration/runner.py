...
from ..validation.bootstrap import check_bootstrap_results, run_bootstrap_test
from ..validation.correlation_analysis import check_correlation_warnings, run_correlation_stress_analysis
from ..validation.permutation import check_permutation_results, run_permutation_test
...
class BacktestRunner:
    ...
    The BacktestRunner coordinates all components needed for backtesting:
    data loading, strategy initialization, engine creation, and result saving.
    ...
    Example:
        ...
        >>>
        >>> # Load configuration
        >>> config = RunConfig.from_yaml("EXAMPLE_CONFIGS/run_config.yaml")
        >>>
        >>> # Create runner
        ...
        >>>
        >>> # Save results
        >>> output_dir = runner.save_results(results, period="train")
    ...
    def load_data(self) -> MarketData:
        ...
        if self.config.strategies.equity and self.config.strategies.equity.enabled:
            ...
            equity_strategy_config = StrategyConfig.from_yaml(self.config.strategies.equity.config_path)
        ...
        if self.config.strategies.crypto and self.config.strategies.crypto.enabled:
            ...
            crypto_strategy_config = StrategyConfig.from_yaml(self.config.strategies.crypto.config_path)
        ...
        benchmark_min_days = (
            1
            if "test" in str(self.config.dataset.benchmark_path).lower()
            or "fixture" in str(self.config.dataset.benchmark_path).lower()
            else 250
        )
    ...
    def load_strategies(self) -> List:
        ...
        strategies = load_strategies_from_run_config(
            equity_config_path=equity_config_path, crypto_config_path=crypto_config_path
        )
    ...
    def run_backtest(
        ...
        Example:
            ...
            >>> results = runner.run_backtest(period="train")
            >>>
            >>> # Run stress test with higher slippage
            >>> stress_results = runner.run_backtest(
            ...     period="validation",
    ...
    def _run_backtest_with_date_filter(
        self, split: WalkForwardSplit, period: str, date_filter: Callable[[pd.Timestamp], bool]
    ) -> Dict[str, Any]:
    ...
    def _get_benchmark_monthly_returns(
        self, start_date: pd.Timestamp, end_date: pd.Timestamp, benchmark_symbol: str = "SPY"
    ) -> Dict[str, float]:
    ...
    def _generate_quarterly_crash_dates(self, start_date: pd.Timestamp, end_date: pd.Timestamp) -> List[pd.Timestamp]:
    ...
    def _extract_benchmark_returns(self, dates: List[pd.Timestamp], benchmark_symbol: str = "SPY") -> Optional[List[float]]:
    ...
    def save_results(self, results: Dict[str, Any], period: str = "train") -> Path:
        ...
        for event in daily_events:
            ...
            equity_curve.append(portfolio_state.get("equity", event.get("equity", self.config.portfolio.starting_equity)))
        ...
        csv_writer.write_weekly_summary(
            equity_curve=equity_curve, dates=dates, daily_returns=daily_returns, closed_trades=closed_trades
        )
        ...
        try:
            ...
            run_id = db.store_results(
                results=db_results, config_path=config_path, strategy_name=strategy_name, period=period, split_name=split_name
            )
...
def run_backtest(config_path: str, period: str = "train") -> Dict[str, Any]:
    ...
    This is the simplest way to run a backtest. It loads configuration,
    initializes the runner, executes the backtest, and saves results.
...
def run_validation(config_path: str) -> Dict[str, Any]:
    ...
    Executes statistical validation tests including bootstrap analysis,
    permutation tests, and stress scenarios.
    ...
    Returns:
        ...
        - rejections: List of failed test names
    ...
    Example:
        ...
        >>> validation_results = run_validation("EXAMPLE_CONFIGS/run_config.yaml")
        >>>
        >>> # Check status
        >>> if validation_results['status'] == 'passed':
        ...     print("✅ Validation passed!")
        >>> else:
        ...     print(f"❌ Validation failed: {validation_results['rejections']}")
    """
    ...
    if not all_closed_trades:
        ...
        return {"status": "failed", "reason": "No closed trades available", "results": {}}
    ...
    for trade in all_closed_trades:
        if trade.exit_price is not None:
            ...
            trades_for_permutation.append(
                {"entry_date": trade.entry_date, "exit_date": trade.exit_date, "symbol": trade.symbol, "r_multiple": r_mult}
            )

    if not r_multiples:
        logger.warning("No valid R-multiples found for validation suite")
        return {"status": "failed", "reason": "No valid R-multiples available", "results": {}}
    ...
    try:
        bootstrap_results = run_bootstrap_test(r_multiples=r_multiples, n_iterations=n_bootstrap, random_seed=random_seed)
    ...
    try:
        ...
        def run_stress_backtest(
            slippage_multiplier: float = 1.0, crash_dates: Optional[Any] = None, date_filter: Optional[Any] = None
        ) -> Dict[str, Any]:
        ...
        stress_suite = StressTestSuite(run_backtest_func=run_stress_backtest, random_seed=random_seed)
    ...
    try:
        ...
        if not portfolio_history:
            ...
            correlation_results = run_correlation_stress_analysis(
                portfolio_history=portfolio_history, returns_data=returns_data
            )
        ...
        validation_results_dict["correlation"] = {"error": str(e), "error_type": type(e).__name__}
...
def run_sensitivity_analysis(
    config_path: str, period: str = "train", metric_name: str = "sharpe_ratio", asset_class: Optional[str] = None
) -> Dict[str, Any]:
    ...
    for asset_class_name, strategy_config_path in strategies_to_analyze:
        ...
        parameter_ranges = generate_parameter_grid_from_config(sensitivity_config, asset_class=asset_class_name)
        ...
        def metric_func(params: Dict[str, Any]) -> float:
            ...
            try:
                ...
                if strategy_params:
                    modified_strategy_config = apply_parameters_to_strategy_config(modified_strategy_config, strategy_params)

                # Apply portfolio parameters
                if portfolio_params:
                    modified_run_config = apply_parameters_to_run_config(modified_run_config, portfolio_params)
        ...
        grid = ParameterSensitivityGrid(
            parameter_ranges=parameter_ranges, metric_func=metric_func, random_seed=config.random_seed
        )
    ...
    return {"status": "completed", "metric_name": metric_name, "period": period, "results": all_results}
