...
class ReportGenerator:
    ...
    def load_period_data(self, period: str) -> Optional[Dict[str, Any]]:
        ...
        data = {"period": period, "directory": period_dir}
    ...
    def compute_metrics_from_data(self, data: Dict[str, Any]) -> Dict[str, float]:
        ...
        if not trade_df.empty:
            for _, row in trade_df.iterrows():
                ...
                initial_stop_price = (
                    float(row["initial_stop_price"]) if pd.notna(row.get("initial_stop_price")) else entry_price * 0.95
                )
                stop_price = (
                    float(row.get("stop_price", initial_stop_price)) if pd.notna(row.get("stop_price")) else initial_stop_price
                )

                # Determine position side (default to LONG for backward compatibility)
                side_str = str(row.get("side", "LONG")).upper()
                side = PositionSide.SHORT if side_str == "SHORT" else PositionSide.LONG

                # Ensure stop_price is valid based on position side
                if side == PositionSide.LONG:
                    ...
                    if initial_stop_price <= entry_price:
                        initial_stop_price = entry_price * 1.05
                ...
                trade = Position(
                    ...
                    exit_reason=(
                        ExitReason(row["exit_reason"]) if pd.notna(row.get("exit_reason")) and row.get("exit_reason") else None
                    ),
        ...
        metrics_calc = MetricsCalculator(
            equity_curve=equity_curve, daily_returns=daily_returns, closed_trades=closed_trades, dates=dates
        )
    ...
    def generate_comparison_report(self, output_path: Optional[Path] = None) -> Path:
        ...
        for metric in key_metrics:
            ...
            comparison["comparison"][metric] = {
                "best": {"period": best_period[0], "value": float(best_period[1])},
                "worst": {"period": worst_period[0], "value": float(worst_period[1])},
