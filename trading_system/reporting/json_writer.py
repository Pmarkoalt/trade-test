...
class JSONWriter:
    ...
    def write_monthly_report(
        ...
        if len(equity_curve) != len(dates):
            raise ValueError(f"equity_curve length ({len(equity_curve)}) must match " f"dates length ({len(dates)})")
        ...
        df = pd.DataFrame({"date": dates, "equity": equity_curve, "daily_return": daily_returns_aligned})
        ...
        daily_returns_for_metrics = (
            daily_returns_aligned[1:] if len(daily_returns_aligned) == len(equity_curve) else daily_returns
        )
        ...
        report = {
            ...
            "overall_metrics": {
                k: float(v) if isinstance(v, (int, float, np.number)) else v for k, v in overall_metrics.items()
            },
    ...
    def write_scenario_comparison(self, scenarios: Dict[str, Dict]) -> str:
        ...
        converted_scenarios = {name: {k: convert_value(v) for k, v in metrics.items()} for name, metrics in scenarios.items()}
    ...
    def _compare_scenarios(self, scenarios: Dict[str, Dict]) -> Dict:
        ...
        for metric in key_metrics:
            ...
            values = {name: metrics.get(metric) for name, metrics in scenarios.items() if metrics.get(metric) is not None}
