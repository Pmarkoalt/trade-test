...
class BacktestVisualizer:
    ...
    def plot_equity_curve(
        ...
        # Plot equity curve
        ax.plot(equity_df["date"], equity_df["equity"], linewidth=2, label="Portfolio Equity", color="#2E86AB")
    ...
    def plot_drawdown(self, period: str = "train", save_path: Optional[Path] = None) -> Figure:
    ...
    def plot_trade_distribution(self, period: str = "train", save_path: Optional[Path] = None) -> Figure:
        ...
        ax1.axvline(np.mean(pnl_values), color="green", linestyle="--", linewidth=2, label=f"Mean: ${np.mean(pnl_values):.2f}")
        ...
        if "r_multiple" in trade_df.columns:
            ...
            ax2.text(0.5, 0.5, "R-multiple not available", ha="center", va="center", transform=ax2.transAxes)
        ...
        if len(sizes) > 0:
            ax3.pie(
                ...
                autopct="%1.1f%%",
                startangle=90,
                explode=explode if len(explode) > 0 else None,
            )
        else:
        ...
        if "holding_days" in trade_df.columns:
            ...
            if len(holding_days) > 0:
                ...
                ax4.text(0.5, 0.5, "No holding period data", ha="center", va="center", transform=ax4.transAxes)
        else:
            ax4.text(0.5, 0.5, "Holding period not available", ha="center", va="center", transform=ax4.transAxes)

        fig.suptitle(f"Trade Distribution Analysis - {period.upper()} Period", fontsize=14, fontweight="bold", y=0.995)
    ...
    def plot_monthly_returns_heatmap(self, period: str = "train", save_path: Optional[Path] = None) -> Figure:
        ...
        monthly_returns = (
            equity_df.groupby(["year", "month"])["return"].apply(lambda x: (1 + x).prod() - 1) * 100
        ...
        # Create heatmap
        im = ax.imshow(monthly_pivot.values, cmap="RdYlGn", aspect="auto", vmin=-10, vmax=10)  # Adjust vmin/vmax as needed
        ...
        ax.set_title(f"Monthly Returns Heatmap - {period.upper()} Period", fontsize=14, fontweight="bold")
    ...
    def plot_all(self, period: str = "train", output_dir: Optional[Path] = None) -> Dict[str, Path]:
...
def plot_equity_curve_from_data(
    equity_curve: List[float], dates: List[pd.Timestamp], save_path: Optional[Path] = None, title: str = "Equity Curve"
) -> Figure:
