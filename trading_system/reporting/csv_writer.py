...
class CSVWriter:
    ...
    def write_weekly_summary(
        self, equity_curve: List[float], dates: List[pd.Timestamp], daily_returns: List[float], closed_trades: List[Position]
    ) -> str:
        ...
        if len(daily_returns_aligned) != len(dates):
            raise ValueError(f"Failed to align daily_returns: expected {len(dates)}, got {len(daily_returns_aligned)}")
