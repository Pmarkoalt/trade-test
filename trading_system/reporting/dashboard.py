...
def render_trade_analysis(data: Dict[str, Any], period: str):
    ...
    pnl_hist = pd.DataFrame({
        'P&L': trade_df['realized_pnl'].values
    })
    st.hist_chart(pnl_hist, x='P&L', bins=50)
    
    # Trade table
    st.write("**Recent Trades**")
    display_cols = ['symbol', 'entry_date', 'exit_date', 'entry_price', 
                   'exit_price', 'quantity', 'realized_pnl']
    available_cols = [col for col in display_cols if col in trade_df.columns]
    
    st.dataframe(
        trade_df[available_cols].tail(20),
...
def render_monthly_returns(data: Dict[str, Any], period: str):
    ...
    st.dataframe(
        monthly_pivot.style.background_gradient(cmap='RdYlGn', vmin=-10, vmax=10),
        use_container_width=True
    )
...
def main():
    ...
    tab1, tab2, tab3, tab4 = st.tabs([
        "Equity Curve", "Drawdown", "Trade Analysis", "Monthly Returns"
    ])
