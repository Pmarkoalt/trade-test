...
class ResultsDatabase:
    ...
    def _store_trades(self, cursor: sqlite3.Cursor, run_id: int, trades: List[Position]) -> None:
        ...
        for trade in trades:
            ...
            exit_date = trade.exit_date.isoformat() if isinstance(trade.exit_date, pd.Timestamp) else (str(trade.exit_date) if trade.exit_date else None)
    ...
    def _store_equity_curve(self, cursor: sqlite3.Cursor, run_id: int, equity_curve: List[float], daily_events: List[Dict]) -> None:
    ...
    def _store_monthly_summary(self, cursor: sqlite3.Cursor, run_id: int, equity_curve: List[float], 
                               daily_returns: List[float], dates: List, closed_trades: List[Position]) -> None:
    ...
    def compare_runs(self, run_ids: List[int], metrics: Optional[List[str]] = None) -> pd.DataFrame:
        ...
        if metrics is None:
            metrics = [
                'sharpe_ratio', 'max_drawdown', 'calmar_ratio', 'total_return',
                'total_trades', 'win_rate', 'avg_r_multiple', 'profit_factor',
                'expectancy', 'turnover'
            ]
    ...
    def archive_runs(self, run_ids: List[int], archive_db_path: Optional[Path] = None) -> None:
        ...
        try:
            ...
            logger.warning("Full archival (trades, equity curves) not implemented - use database backup tools for complete archival")
        finally:
            conn.close()
